...
# --- 1. 設定網頁標題與版面 ---
st.set_page_config(page_title="申請研究所自傳生成系統", page_icon="🎓", layout="centered")
...
with st.form("autobiography_form"):
    ...
    st.caption("""請輸入您的核心特質關鍵字，以及最具代表性的經歷。""")
    
    st.markdown("👉 **個人特質關鍵字 (請輸入 3 個形容詞)**")
...
if submitted:
    ...
    if not name or not highest_school or not target_school or not target_dept:
        ...
        if traits_list:
            ...
            traits_sentence = "我具備積極進取且樂於挑戰的個人特質。" # 預設值
        ...
        lines.append(f"{traits_sentence}我的處事觀念是「不做不會怎樣，做了很不一樣」，並始終以「事情要做到最好」要求自己。在成長過程中，我因 **{story}**，培養出強大的**抗壓性與執行力**。這段經歷使我具備獨立思考的能力，並能以樂觀積極的態度面對學術研究上的挑戰。")
        lines.append("")
